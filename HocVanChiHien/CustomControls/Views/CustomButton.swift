...
//  DerivativeIOS
//
//  Created by ChacND_HAV on 3/14/19.
...
class CustomButton: UIButton {
    ...
    var image: UIImage {
        set (newValue) {
            _image = newValue
            self.setImage(_image.withRenderingMode(UIImage.RenderingMode.alwaysTemplate), for: UIControl.State.normal)
        } get {
            return _image
        }
    }
    ...
    var imageMulti: CGFloat {
        set (newValue) {
            ...
            self.imageEdgeInsets = UIEdgeInsets(top: self.frame.height * (1 - _imageMulti) / 2,
